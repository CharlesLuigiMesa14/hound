            ...
            <div class="col-md-6 col-lg-4">
                <div class="card" style="background-color: rgba(255, 255, 255, 0.95); border-radius: 10px; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.5);">
                    ...
                    <div class="card-body" style="color: #333; font-family: 'Roboto', sans-serif;">
                        <form method="POST" action="{{ route('register.admin') }}" onsubmit="return validateForm()">
                            @csrf
                            ...
                            <!-- Role Selection -->
                            <div class="mb-3">
                                <label for="role_as" class="form-label" style="font-weight: bold; font-size: 0.9rem;">{{ __('Select Role') }}</label>
                                <select id="role_as" name="role_as" class="form-select" required>
                                ...
                                <small class="form-text text-danger" id="role-alert" style="display: none;">Please select a role!</small>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <input id="password-confirm" type="password" class="form-control" name="password_confirmation" required autocomplete="new-password" placeholder="Confirm Password" style="border: 1px solid #ccc; padding: 10px; font-size: 0.9rem;" oninput="validatePassword()">
                                </div>
...
function validateForm() {
    ...
    const roleValid = document.getElementById('role_as').value !== ""; // Ensure role is selected
    const passwordValid = password.length >= 6; // Ensure password is at least 6 characters

    // Enable button only if all fields are valid
    submitBtn.disabled = !(nameValid && lnameValid && emailValid && roleValid && passwordValid);
    return true;
}
...
document.getElementById('role_as').addEventListener('change', validateForm); // Use correct ID for role selection
</script>
@endsection